...
function iniciaHamburguesa() {
    ...
    // Ocultar menú hamburguesa al hacer clic en un enlace
    $navbarNav.find('a').on('click', function() {
            // Si es móvil
    if (window.innerWidth <= 768) {
        // Si el enlace es un dropdown-toggle, NO cierres el menú hamburguesa
        if ($(this).hasClass('dropdown-toggle')) {
            // Solo despliega el dropdown, no cierres el menú
            return;
        }
        // Si el enlace está dentro de un dropdown abierto, NO cierres el menú hamburguesa
        if ($(this).closest('.dropdown-menu').length > 0) {
            // Permite cerrar el menú hamburguesa solo si quieres, o déjalo abierto
            // return; // Descomenta para que tampoco se cierre al hacer clic en un dropdown-item
        }
    ...
    // Cierra los dropdowns al hacer clic en un dropdown-item
    $navbarNav.find('.dropdown-item').on('click', function() {
        $('.nav-item.open').removeClass('open');
    });

    // Al cambiar tamaño de ventana, si es escritorio, asegúrate de mostrar el menú y el icono correctamente
...
// Ejecuta iniciaNav al cargar la página
document.addEventListener('DOMContentLoaded', function() {
    iniciaHamburguesa();
    iniciaNav();
});
// Ejecuta iniciaNav cada vez que cambie el tamaño de la ventana
window.addEventListener('resize', iniciaNav);

// Ejecuta iniciaHamburguesa al cargar la página index.html
